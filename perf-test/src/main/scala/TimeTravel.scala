...
object TimeTravel {
  def apply(preRequisite: PreRequisite): Unit = {
    ...
    val deliveries: List[Delivery] = Gen.listOf[Delivery](Generator.delivery).pureApply(deliveryParameters, Seed.random)
    ...
    val clicks: List[Click] = deliveries
      .map(d => Gen.listOf[Click](Generator.click(d.deliveryId)).pureApply(clickParameters, Seed.random))
      .flatten
    ...
    val installs: List[Install] = 
      clicks
        .map(c => Gen.listOf[Install](Generator.install(c.clickId)).pureApply(installParameters, Seed.random))
        .flatten
