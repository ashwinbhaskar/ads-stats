...
class StatsRepositoryImpl(transactor: Transactor[IO]) extends StatsRepository {
    ...
    override def recordDelivery(d: Delivery): RepoResult[Unit] =
        sql"INSERT INTO delivery(delivery_id, advertisement_id, t, browser, os, site) VALUES (${d.deliveryId}, ${d.advertisementId},${d.time},${d.browser},${d.os},${d.site})"
            .update
            .run
            .transact(transactor)
    ...
    override def getStats(start: OffsetDateTimeWithoutMillis, end: OffsetDateTimeWithoutMillis)
        : RepoResult[Stats] = {
            val result: IO[Stats] = 
                for {
    ...
    override def getStats(start: OffsetDateTimeWithoutMillis, end: OffsetDateTimeWithoutMillis,
        categories: List[Category]): RepoResult[List[CategorizedStats]] = {
            val result: IO[List[CategorizedStats]] = 
                for {
                    deliveries <- sql"SELECT advertisement_id, delivery_id, t, browser, os, site FROM delivery WHERE t BETWEEN $start AND $end"
                                    .query[Delivery].to[List].transact(transactor)
                    clicks <- sql"SELECT delivery_id, click_id, t FROM click WHERE t BETWEEN $start AND $end"
                                    .query[Click].to[List].transact(transactor)
                    installs <- sql"SELECT install_id, click_id, t FROM install WHERE t BETWEEN $start AND $end"
                                    .query[Install].to[List].transact(transactor)
                } yield {
                    val grouping: Map[List[String], List[Delivery]] = 
                        if(categories.contains(OSCategory) && categories.contains(BrowserCategory))
                    ...
                    grouping.map{
                        case (key, values) => 
                            ...
                            key match {
                                case List(f1, f2) => 
                                    CategorizedStats(Map(categories(0) -> f1, categories(1) -> f2), stats)
                                case List(f1) => 
                                    CategorizedStats(Map(categories.head -> f1), stats)
                            }
                        }.toList
                }
            result.redeem(_ => UnhandledError.asLeft[List[CategorizedStats]], cs => cs.asRight[Error])
        }   
}
